...
func TestReElection2A(t *testing.T) {
	...
	fmt.Println("Phase 0 leader ", leader1)
	// if the leader disconnects, a new one should be elected.
	fmt.Println("disconnect leader1 ", leader1)
	cfg.disconnect(leader1)
	tmpLeader := cfg.checkOneLeader()
	fmt.Println("Phase 1 leader", tmpLeader)
	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	fmt.Println("connect leader1 ", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	fmt.Println("Phase 2 leader ", leader2)
	// if there's no quorum, no leader should
	// be elected.
	fmt.Println("disconnect leader2", leader2, " and leader2 + 1", leader2+1)
	...
	fmt.Println("connect leader2 + 1 ", leader2+1)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	fmt.Println("Phase 4")

	// re-join of last node shouldn't prevent leader from existing.
	fmt.Println("connect leader2 ", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
